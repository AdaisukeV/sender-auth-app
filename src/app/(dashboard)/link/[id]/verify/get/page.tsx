...
export default function VerifyDomain() {
  ...
  useEffect(() => {
    const fetchDNSRecords = async () => {
      try {
        const response = await axios.get(`/whitelabel/links/${id}`);
  ...
  const handleVerify = async () => {
    try {
        const response = await axios.post(`/whitelabel/links/${id}/validate`);
  ...
  const handleDelete = async () => {
    try {
        await axios.delete(`/whitelabel/links/${id}/delete`);
        router.push('/');
    } catch (error) {
        console.error(error);
    }
  ...
  return (
    <div>
      <div className='flex items-center justify-between'>
        <h1 className='my-4'>
          <div className='text-lg'>Link Branding</div>
          {dnsRecords.length > 0 ? (
